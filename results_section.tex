...
battery, the charging curve, and the station access overheads, while keeping the
node set, the customer windows and the full Hours-of-Service machinery intact.
Former charging stations remain as break-eligible waypoints, so the diesel
problem is a genuine relaxation of the electric one on the same corridor and the
same travel-time realization, and the two are solved by the same oracle and the
same greedy policy.  All 160 instance pairs completed in both worlds, and every
oracle solve closed to within 0.5\%.
...
makespan, i.e.\ the penalty predicted by a planner who ignores that charging can
share time with a mandatory rest.  ``Coupling'' is the share of charging time
credited inside a mandatory break, $\sum_i g_i / \sum_i \tau^c_i$, at the
hindsight optimum.  $n = 80$ instances per route class.}
\label{tab:diesel}
\begin{tabular}{lrrrrrrr}
\toprule
\textbf{Route} & \textbf{Diesel (h)} & \textbf{EV (h)} & \textbf{Charging (h)}
 & \textbf{Naive (\%)} & \textbf{Oracle (\%)} & \textbf{Greedy (\%)} & \textbf{Coupling (\%)} \\
\midrule
Short  & 26.4 & 28.7 & 2.69 & $+10.2$ & $+8.6$  & $+20.7$ & 76 \\
Medium & 52.4 & 58.2 & 6.80 & $+13.0$ & $+11.1$ & $+24.2$ & 70 \\
\bottomrule
\end{tabular}
\end{table}
...
corridors: $+8.6\%$ on the short routes and $+11.1\%$ on the medium ones at the
hindsight optimum, positive on every one of the 160 instances (range $+2.9$ to
$+28.0\%$).  Break/charge coupling is effective in the sense the model
predicts, since 70--76\% of charging time is credited inside a mandatory break,
yet it removes only about 15\% of the naive penalty, which is $+10.2\%$ and
$+13.0\%$.  Decomposing the short-route penalty of 2.24\,h shows why:
...
Charging-station manoeuvring        & $+0.55$ & $+1.38$ \\
Break time no longer taken separately & $-1.28$ & $-3.35$ \\
Rest time                            & $-0.05$ & $+0.18$ \\
\midrule
\textbf{Net penalty vs.\ diesel}     & $\mathbf{+2.24}$ & $\mathbf{+5.79}$ \\
\bottomrule
\end{tabular}
\end{center}

The coupling credit, that is the break time the driver no longer has to take
separately, is 1.28\,h and 3.35\,h, but it is bounded above by the break
duration that the regulation requires in the first place: charging for longer
than 45~minutes at a stop earns no further credit.  Meanwhile the electric
truck pays 0.88\,h and 2.16\,h of pure station-access overhead in queueing and
manoeuvring, which are absent for diesel and which consume roughly two thirds of
the credit.  The net effect is that optimal scheduling recovers about a sixth of
the naive estimate rather than all of it.  This qualifies, rather than
contradicts, the finding of \citet{brockmann_break-and-charge_2025} that
mandatory breaks create natural charging windows: the windows exist and the
model exploits them, but the access overheads of a fast-charging stop, which
that work does not price, absorb most of the gain.

The greedy comparison isolates the remaining half of the story.  Greedy's
electric schedule is 20.7\% longer than its own diesel schedule on the short
routes and 24.2\% longer on the medium ones, roughly double the optimal
penalty.  The excess is not electrification but planning quality: measured
against its own hindsight optimum, Greedy loses 13.2\% (short) and 14.1\%
(medium) on the electric problem, but only 2.0\% and 1.8\% on the diesel
...
thumb costs a further 12 percentage points of makespan.  Of the roughly 21--24\%
that an operator would observe when switching a myopically-scheduled corridor
from diesel to battery-electric, about half is the physical cost of charging
and about half would be recovered by adopting a planner such as the ones
