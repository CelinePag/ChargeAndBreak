...
SAFETY_BUF = 25.0     # kWh on top of Emin+energy-to-next-CS for the clamp
                      # (5% of the 500 kWh pack; ablatable, logged when it bites)
...
def make_policy(ckpt_path: str, raw_inst: dict, full_data: dict, stats: dict):
    ...
    def policy(fd, stop, vehicle):
        ...
        x_raw = torch.tensor([features_at(ctx, st)], dtype=torch.float32)
        ...
        # 2. safety clamp on the charge duration (only when charging)
        if y == 1:
            need    = Emin + ctx["e_next_cs"][stop] + SAFETY_BUF   # kWh target
        ...
        # 3. durations, mirroring vehicle.advance()'s fallback semantics
...
def instances_for(splits: list[str]) -> list[str]:
    out = []
    for f in sorted(glob.glob(os.path.join(INST_DIR, "R*.json"))):
        name = os.path.basename(f)[:-5]
        m = re.match(r"(R[a-z]+C[a-z]+T[a-z]+)_(\d+)$", name)
        if m and split_of(m.group(1), int(m.group(2))) in splits:
            out.append(name)
    return out


def main(a):
    stats = dict(clamp_up=0, tauc_pred_h=[])
    names = instances_for(a.split)
    ...
    for k, name in enumerate(names):
        ...
        full_data, D_real, E_real, _cv = load_instance_json(
    ...
    print(f"\ncharge decisions: {n_chg} | safety clamp raised tauc on "
