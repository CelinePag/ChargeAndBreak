...
# NOTE: these files were restored from git into ML/legacy/code/, one
# directory deeper than the ML/code/ they were written for, so ROOT
# needs one more dirname than the original three.
...
def load_split(which: tuple[str, ...], k: int, mode: str):
    """Rows for the requested splits, plus a per-row weight.

    DAgger is intentionally NOT restored: only the base behavioural-
    from the state distribution the student actually produces, so they are
    cloning deck is loaded, and every row has weight 1.
    """
...
def main(a):
    ...
    Xtr, ytr, ttr, str_ = load_split(("train",), a.k, a.mode)
    Xva, yva, tva, _ = load_split(("val",), a.k, a.mode)
    # per-row weights: uniform (DAgger removed)
