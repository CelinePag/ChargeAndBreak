...
Usage
  python recover_variant_oracles.py --dry-run
  python recover_variant_oracles.py
"""
...
def quarantine_base(dry: bool) -> int:
    ...
    for cb in COMBOS:
        for tw in TWS:
            for s in SEEDS:
                p = os.path.join("solutions", f"oracle_{cb}T{tw}_{s}.json")
...
def rebuild_variants(dry: bool) -> tuple[int, int]:
    ...
    for p in sorted(glob.glob(os.path.join("logs", "*__*_ORACLE_*.txt"))):
    ...
    for stem, (_ts, rec) in best.items():
        out = os.path.join("solutions", f"oracle_{stem}.json")
        if dry:
