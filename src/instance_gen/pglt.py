...
Usage
-----
  python -m src.instance_gen.pglt all                 solve all 40, compare, write CSV
  python -m src.instance_gen.pglt 1,5,13              solve a subset (instance numbers)
  python -m src.instance_gen.pglt Test_7 --tee        single instance with solver log
  python -m src.instance_gen.pglt --scan-reference    audit their solution files (grid/APO/splits)
...
from src.methods import MILP
from src.instance_gen.instances import make_data
from src import paths as _paths

DATA_DIR = _paths.data("R15-PGLT")
...
def main() -> None:
    ...
    ap.add_argument("--out", default=_paths.data_output("pglt_comparison.csv"))
    args = ap.parse_args()
