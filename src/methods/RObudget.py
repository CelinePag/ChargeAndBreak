...
-----------
  ...
  Or via runner_dispatch:
    python -m src.simulation.runner_dispatch instances/RmediumCfewTmedium_7.json ROBU
"""
...
if __name__ == "__main__":
    ...
    # Usage: python -m src.methods.RObudget <json_file> [time_limit_s] [gamma]
    json_file = sys.argv[1] if len(sys.argv) > 1 else None
    time_lim  = int(sys.argv[2]) if len(sys.argv) > 2 else 2 * 3600
    gamma_cli = int(sys.argv[3]) if len(sys.argv) > 3 else None

    if json_file is None:
        print("Usage: python -m src.methods.RObudget <json_file> [time_limit_s] [gamma]")
        sys.exit(1)
