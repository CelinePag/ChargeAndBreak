...
Usage
  python audit_runs.py                 # report only (default)
  python audit_runs.py --apply         # move offending files to archive/
  python audit_runs.py --apply --logs  # also archive orphan logs
"""
...
def instance_index() -> dict[str, str]:
    ...
    idx = {}
    for p in glob.glob(os.path.join("instances", "*.json")):
        idx[os.path.splitext(os.path.basename(p))[0]] = p
    for p in glob.glob(os.path.join("instances_sens", "*", "*.json")):
...
def main() -> None:
    ...
    if args.apply:
        ...
        print("\nre-run `python audit_runs.py` to confirm a clean bill.")
    else:
