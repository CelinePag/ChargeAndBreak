...
Usage
  python fix_variant_titles.py --dry-run     # report only
  python fix_variant_titles.py               # apply
"""
...
def retitle_instances(dry: bool) -> int:
    n = 0
    for path in glob.glob(os.path.join("instances_sens", "*", "*.json")):
...
def quarantine_runs(dry: bool) -> int:
    ...
    n = 0
    for path in glob.glob(os.path.join("solutions", "*.json")):
...
def suspect_oracles() -> list[str]:
    ...
    var = [p for p in glob.glob(os.path.join("solutions", "*.json"))
    ...
    return [os.path.basename(p)
            for p in glob.glob(os.path.join("solutions", "oracle_*.json"))
